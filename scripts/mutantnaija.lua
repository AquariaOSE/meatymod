...
-- FG: hacked to handle "reinit" msg, poison immunity, no damage from other mutants, and increased updatecull radius
...
function init(me)
	...
	entity_initSkeletal(me, "naija-mutant", "mutant")
	--entity_setAllDamageTargets(me, false)
    entity_setDamageTarget(me, DT_ENEMY_POISON, false) -- FG: added these
    entity_setDamageTarget(me, DT_ENEMY_ACTIVEPOISON, false)
    ...
    -- FG: no longer necessary, these were removed from the animation file
...
function damage(me, attacker, bone, damageType, dmg)
    -- FG: added that. self-damage is for entity_hugeDamage(), rest is to prevent them from killing each other
	if attacker ~= 0 and attacker ~= me and entity_getName(attacker) == "mutantnaija" then
        return false
    end
